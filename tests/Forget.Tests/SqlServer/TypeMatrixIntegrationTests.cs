...
{
    /// <summary>
    /// Proves every CLR type in <see cref="TypeMatrixRow"/> survives a real SqlServer round trip through the
    ...
    public class TypeMatrixIntegrationTests : IClassFixture<SqlServerFixture>
    {
        ...
        public TypeMatrixIntegrationTests(SqlServerFixture fixture)
        {
            _fixture = fixture;
        }
        ...
        private static TypeMatrixRow Sample(int id) => new()
        {
            ...
            // Dapper binds a DateTime as the legacy "datetime" type (1/300 s resolution), so sub-millisecond digits
            // would be rounded before reaching the datetime2 column; 120 ms is exactly representable. Forget leaves
            // this to Dapper on purpose (remedy: SqlMapper.AddTypeMap(typeof(DateTime), DbType.DateTime2)).
            Moment = new DateTime(2026, 9, 19, 12, 34, 56, DateTimeKind.Unspecified).AddMilliseconds(120),
            OffsetMoment = new DateTimeOffset(2026, 9, 19, 12, 34, 56, TimeSpan.FromHours(2)).AddTicks(1_234_560),
        ...
        private static void ChangeEverything(TypeMatrixRow row)
        {
            ...
            row.OffsetMoment = new DateTimeOffset(2000, 1, 1, 0, 0, 0, TimeSpan.FromHours(-5));
