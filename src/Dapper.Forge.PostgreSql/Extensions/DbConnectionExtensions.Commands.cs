...
namespace Dapper.Forge.PostgreSql.Extensions
{
    public static partial class DbConnectionExtensions
    {
        ...
        /// </exception>
        public static DbCommandInfo UpdateCommand<TEntity>(this NpgsqlConnection connection, object values) where TEntity : class
        {
            DbCommandStrategy.Instance.LoadRuntimeCache<TEntity>(connection);
        ...
        /// <paramref name="predicate"/> uses an expression shape that the SQL translator does not support.
        /// </exception>
        public static DbCommandInfo UpdateCommand<TEntity>(this NpgsqlConnection connection, object values, Expression<Func<TEntity, bool>>? predicate) where TEntity : class
        {
            DbCommandStrategy.Instance.LoadRuntimeCache<TEntity>(connection);
        ...
        /// translator does not know how to translate it.
        /// </exception>
        public static DbCommandInfo UpdateCommand<TEntity>(this NpgsqlConnection connection, object values, IFilterNode<TEntity>? filterNode) where TEntity : class
        {
            DbCommandStrategy.Instance.LoadRuntimeCache<TEntity>(connection);
        ...
        /// <remarks>
        /// The generated command is an <c>INSERT ... ON CONFLICT (...) DO UPDATE</c> statement, where the conflict
        ...
        /// matching <c>UNIQUE</c> or exclusion constraint in the database; if none exists, executing the command
        /// fails with a database error rather than silently inserting a duplicate. On a match, every property is
        ...
        /// <para>
        /// Because conflict detection relies on the table's actual constraint, whether a <see langword="null"/> key
        /// value is treated as matching another <see langword="null"/> follows standard PostgreSQL behavior: by
        /// default, <see langword="null"/> values never conflict with each other, so a row with a
        /// <see langword="null"/> key is always inserted rather than merged, unless the constraint itself is
        /// defined to treat nulls as not distinct.
        /// </para>
        /// </remarks>
        /// <exception cref="ArgumentNullException">
        /// <paramref name="connection"/> or <paramref name="entity"/> is <see langword="null"/>.
        ...
        /// </returns>
        /// <remarks>
        /// Each generated command is an <c>INSERT ... ON CONFLICT (...) DO UPDATE</c> statement, where the conflict
        /// target is the properties marked with <see cref="UpsertKeyAttribute"/> — or the
        /// entity's identifier, if none are marked. PostgreSQL requires those columns to already be covered by a
        /// matching <c>UNIQUE</c> or exclusion constraint in the database; if none exists, executing the command
        /// fails with a database error rather than silently inserting a duplicate. On a match, every property is
        /// updated except the identifier, any database-generated property, and the key properties themselves;
        /// otherwise, a new row is inserted using every property that is not database-generated. By default,
        /// <see langword="null"/> key values never conflict with each other, so entities with a
        /// <see langword="null"/> key are always inserted rather than merged, unless the constraint itself is
        /// defined to treat nulls as not distinct.
        /// </remarks>
        /// <exception cref="ArgumentNullException">
        /// <paramref name="connection"/> or <paramref name="entities"/> is <see langword="null"/>.
        ...
        /// <returns>The <see cref="DbCommandInfo"/> for the query.</returns>
        /// <remarks>
        /// The generated command casts the computed average to a string using PostgreSQL's <c>::text</c> cast;
        /// parsing it back into a <see cref="decimal"/> is the responsibility of whichever code executes the
        /// command (the library's own execution methods do this automatically).
        /// </remarks>
        /// <exception cref="ArgumentNullException">
        /// <paramref name="connection"/> or <paramref name="selector"/> is <see langword="null"/>.
        /// </exception>
        ...
        /// <paramref name="predicate"/> uses an expression shape that the SQL translator does not support.
        /// </exception>
        public static DbCommandInfo MinCommand<TEntity, TProperty>(this NpgsqlConnection connection, string propertyName, Expression<Func<TEntity, bool>>? predicate) where TEntity : class
        {
            DbCommandStrategy.Instance.LoadRuntimeCache<TEntity>(connection);
        ...
        /// translator does not know how to translate it.
        /// </exception>
        public static DbCommandInfo MinCommand<TEntity, TProperty>(this NpgsqlConnection connection, string propertyName, IFilterNode<TEntity>? filterNode) where TEntity : class
        {
            DbCommandStrategy.Instance.LoadRuntimeCache<TEntity>(connection);
        ...
        /// <paramref name="predicate"/> uses an expression shape that the SQL translator does not support.
        /// </exception>
        public static DbCommandInfo MaxCommand<TEntity, TProperty>(this NpgsqlConnection connection, string propertyName, Expression<Func<TEntity, bool>>? predicate) where TEntity : class
        {
            DbCommandStrategy.Instance.LoadRuntimeCache<TEntity>(connection);
        ...
        /// translator does not know how to translate it.
        /// </exception>
        public static DbCommandInfo MaxCommand<TEntity, TProperty>(this NpgsqlConnection connection, string propertyName, IFilterNode<TEntity>? filterNode) where TEntity : class
        {
            DbCommandStrategy.Instance.LoadRuntimeCache<TEntity>(connection);
